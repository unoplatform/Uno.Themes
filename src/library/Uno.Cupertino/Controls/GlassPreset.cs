...
/// <param name="BandLight">Alpha of the white cast over the refracting band between rim and body, 0–1; the light the bevel catches.</param>
...
internal readonly record struct GlassPreset(float Sigma, float Saturation, float Refraction, float Rim, float Veil, float Shadow = 0, float InnerShadow = 0, float RimWidth = 1, float BandLight = 0)
{
	...
	public static GlassPreset For(GlassMaterial material) => material switch
	{
		...
		GlassMaterial.Clear => new(Sigma: 0, Saturation: 1.1f, Refraction: 7, Rim: 0.95f, Veil: 0f, Shadow: 0.22f, InnerShadow: 0.26f, RimWidth: 2, BandLight: 0.45f),
