...
- **Runtime-test placement:** New tests belong under `src/samples/Uno.Themes.RuntimeTests/`, ideally extending an existing `Given_*` file rather than creating a parallel one. Tests added elsewhere (or in a fresh top-level project) silently won't run under `build/scripts/linux-skia-desktop-runtime-tests.sh`.
