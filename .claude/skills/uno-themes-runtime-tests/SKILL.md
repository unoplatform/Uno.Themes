...
- **`UNO_RUNTIME_TESTS_OUTPUT_PATH` not exported** (passing only `--runtime-tests=<path>`) → the runner prints `Application has not been configured with output destination, aborting runtime-test embedded runner`, then **launches the normal sample UI and sits there**. The process never exits on its own, produces no results file, and in Release prints nothing else (`InitializeLogging` is `#if DEBUG`), so it looks like a hang rather than a misconfiguration. Export the env var **and** pass the argument, as the blocks above do. On a silent, result-less run, always check line 2 of stdout before suspecting the environment.
- **`UNO_RUNTIME_TESTS_RUN_TESTS` set to a bare class name** (e.g. `Given_FluentTypography` instead of `'{"Filter":{"Value":"Given_FluentTypography"}}'`) → it is **JSON**, not a plain string; a malformed value aborts the runner the same silent way. A good run echoes the parsed config: `Application configured to start runtime-tests (OutputPath=… | Config={…})`.
- **No X display / WSL** → `xvfb-run` + `fluxbox` is what CI uses, but any real display works: on WSLg, `DISPLAY=:0` alone runs the suite green. The `_NET_WM_STATE does not exist … use an EWMH-compliant WM` lines WSLg emits are noise, not a cause of failure.
