...
Motivation: hosts that run several applications (e.g. Hot Design's ALC-hosted guest apps, the ALC wrapper sample in `specs/05-alc-wrapper-app/`) cannot reach a guest's theme through `Application.Current`, which is the host application. `Application.Current` is a process-wide static in the *shared* `Uno.UI` and is deliberately never assigned for a secondary ALC, so a hosted guest reading it gets the host. A caller that resolves the guest's `Application` instance can fetch its theme directly instead.

**`GetTheme()` must be called from inside the guest's own ALC.** Whether it works across an ALC boundary depends entirely on how the host resolves `Uno.Themes.WinUI`, and the wrapper sample in `specs/05-alc-wrapper-app/` deliberately **isolates** it per-ALC (`!Uno.Themes.WinUI` in `ThemesSampleApp/GuestHosting/GuestSharedAssemblies.txt`, where isolation wins over sharing, so that repo-built theme libraries under test are deterministic). Under that policy `BaseTheme` type identity is *not* shared: a host-side `guestApp.GetTheme()` matches the host's own `BaseTheme` type against dictionaries created from the guest's, finds nothing, and returns `null` — silently. Host code that must inspect a guest's theme has to match by type name via reflection (see `GuestHostingSmoke.CheckGuestThemeIsReachable`).

The supported pattern is therefore for the guest to resolve its own `Application` from inside its own ALC. The sample heads do this by publishing themselves on `NavigationHelper.CurrentApplication` — a static in `SamplesApp.Shared`, which is *compiled into* each head and so is per-ALC — and `SeedColorSamplePage` reads the theme from there rather than from `SemanticThemeHelper`. Before that hand-off existed, the Seed Color page threw `InvalidOperationException` out of its own constructor and could not be opened in any hosted guest.

Runtime coverage: `Given_ApplicationExtensions` in `src/samples/SimpleSampleApp/RuntimeTests/` (standalone); the hosted path is gated by the wrapper's `--smoke` harness, which asserts each guest publishes its own `Application` and resolves its own theme from it.
