...
### Button/ToggleButton typeface aliases had to move into `ThemeDictionaries`

`When_DefaultFontFamilySet_Then_ThemeAliasKeysFollow` failed for `SimpleButtonFontFamily` and
`SimpleToggleButtonFontFamily` (CI build 232029: 6 failures = these 2 x the runner's 3 attempts)
while passing for `DatePickerFlyoutPresenterFontFamily`, though all three are in the same
`FontFamilyAliasKeys` list and regenerated by the same loop.

The difference was where each was declared. The two failing keys sat at the **top level** of
`Button.xaml` / `ToggleButton.xaml` under a "theme-agnostic" heading; the passing one sits inside
`ResourceDictionary.ThemeDictionaries`. `BaseTheme` sets its own `Source` to `mergedpages.xaml`
(`BaseTheme.cs:505`), so a top-level key there is one of the theme dictionary's *own* resources, and
own resources beat `MergedDictionaries`. The generated layer arrives via `AddThemeDictionary`, i.e.
merged — it can shadow a themed declaration but never a non-themed one.

Fix: declare both aliases inside the Light and Default blocks the two files already have, matching
`DatePicker` and all twelve of Material's alias keys (verified: every Material alias is themed, so
Material was never exposed to this).

Mirroring the aliases into the generated dictionary's top level was tried first and **does not
work** — that dictionary is merged, so its non-themed entries lose to the theme's own. Reverted.

This depends on `0f9976c7`: while the setters still read `{StaticResource SimpleButtonFontFamily}`,
moving the declaration out of the top level would have left a parse-time reference dangling. With
all four consumers on `{ThemeResource}`, the move is safe — the two commits are complementary halves,
consumption and declaration.

Worth noting the two paths disagreed: after `0f9976c7` the rendered-button test
(`When_DefaultFontFamilyChanges_Then_StyledButtonFollows`) passed while the token test still failed,
in the same run. The `{ThemeResource}` setter resolved the generated themed entry; a direct
`Resources.TryGetValue` still hit the top-level snapshot.

Simple runtime tests, `net10.0-desktop` Release headless: 256 cases, 255 passed, 0 failed,
1 pre-existing skip (was 253 / 2 failed / 1). No new build warnings (115, unchanged).
